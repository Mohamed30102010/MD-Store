...
import { createOrder, attachStripeSession, type NewOrderItem } from "@/lib/orders";
...
import { createCheckoutSession, isCardPaymentEnabled } from "@/lib/stripe";

export type CheckoutState = {
  error?: string;
  ok?: boolean;
  orderNumber?: string;
  redirectUrl?: string; // موجودة لما الدفع بالبطاقة — العميل لازم يتحوّل لـ Stripe
};
...
export async function placeOrderAction(
  ...
  const validMethods = isCardPaymentEnabled()
    ? ["cash", "transfer", "card"]
    : ["cash", "transfer"];
  if (!validMethods.includes(paymentMethod))
    return { error: "اختار طريقة دفع." };
  if (paymentMethod === "card" && !isValidEmail(email))
    return { error: "الإيميل مطلوب للدفع بالبطاقة (لإرسال الإيصال)." };
  ...
  // 6) الدفع بالبطاقة — ننشئ جلسة Stripe ونرجّع رابط التحويل للعميل
  if (paymentMethod === "card") {
    try {
      const session = await createCheckoutSession({
        ...
        lines: items.map((i) => ({ name: i.name, priceCents: i.priceCents, qty: i.qty })),
        shippingCents,
        customerEmail: email || current?.email || null,
      });
      if (!session) return { error: "الدفع بالبطاقة مش متاح حاليًا. اختار طريقة تانية." };
      await attachStripeSession(order.id, session.sessionId);
      return { ok: true, orderNumber: order.orderNumber, redirectUrl: session.url };
    } catch {
      return { error: "حصل خطأ أثناء تجهيز صفحة الدفع. جرّب تاني أو اختار طريقة تانية." };
    }
  }
