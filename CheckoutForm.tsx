...
export default function CheckoutForm({
  ...
  // عند نجاح الطلب: نفرّغ السلة، ولو الدفع بالبطاقة نحوّل لـ Stripe، غير كده نروح لصفحة الطلب
  useEffect(() => {
    ...
    <form action={formAction} className="flex flex-col gap-6">
        ...
        >
          ...
          {cardEnabled && (
            <PayOption
              ...
              current={method}
              onSelect={setMethod}
              title="بطاقة (دفع أونلاين)"
              desc="فيزا / ماستركارد — دفع فوري وآمن"
              icon="💳"
            />
          )}
        </div>
      ...
      <div className="rounded-2xl border border-line bg-surface p-5">
        ...
        <p className="mt-2 text-center text-xs text-muted">
          {method === "card"
            ? "هيتم تحويلك لصفحة دفع Stripe الآمنة."
...
function PayOption({
  ...
  current: string;
  onSelect: (v: "cash" | "transfer" | "card") => void;
  title: string;
  desc: string;
  icon: string;
}) {
